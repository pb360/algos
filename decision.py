...
actions_to_bull_or_bear_dict = {'exit_short': 'bullish',
                                'long': 'bullish',
                                'exit_long': 'bearish',
                                'short': 'bearish', }
...
def update_state_dict(state_dict, action, mode='backtest'):
    ...
    if action in ['exit_long', 'short', 'start', ]:
        state_dict['bullish'] = {
            'lowest_pred': np.array([100]),
            'highest_pred': np.array([-100]),
            'lowest_price': 7777777777.77,
            'highest_price': -7777777777.77,
            'activations': {'exit_short': {'threshold': {'state': False, 'cool_down': 0},
                                           'pred_dist': {'state': False, 'cool_down': 0},
                                           'price_dist': {'state': False, 'cool_down': 0},
                                           'stop_limit': {'state': False, 'cool_down': None}, },
                            'long': {'threshold': {'state': False, 'cool_down': 0},
                                     'pred_dist': {'state': False, 'cool_down': 0},
                                     'price_dist': {'state': False, 'cool_down': 0},
                                     'stop_limit': {'state': False, 'cool_down': None}, }, }, }

    if action in ['exit_short', 'long', 'start', ]:  # DO NOT CHANGE TO `elif` because neutral looks both ways
        state_dict['bearish'] = {
            'lowest_pred': np.array([100]),
            'highest_pred': np.array([-100]),
            'lowest_price': 7777777777.77,
            'highest_price': -7777777777.77,
            'activations': {'exit_long': {'threshold': {'state': False, 'cool_down': 0},
                                          'pred_dist': {'state': False, 'cool_down': 0},
                                          'price_dist': {'state': False, 'cool_down': 0},
                                          'stop_limit': {'state': False, 'cool_down': None}, },
                            'short': {'threshold': {'state': False, 'cool_down': 0},
                                      'pred_dist': {'state': False, 'cool_down': 0},
                                      'price_dist': {'state': False, 'cool_down': 0},
                                      'stop_limit': {'state': False, 'cool_down': None}, }, }, }


def handle_triggered_transaction(state_dict, num_transacts, transacts_list, transacts, action, i, dti, price,
                                 triggers='debug_off'):
...
def get_state(price, pred, position, action, switch_name, requests_dict, state_dict, ):
...
def run_decision_algo(preds, prices, requests_dict, debug_triggers=False, max_transacts=10_000):
    ...
    num_transacts = 0
    transacts_list = []
    transacts_dict_of_lists = {'idxs': [],
                               'datetimes': [],
                               'prices': [],
                               'triggers': [], }

    transacts = {'long': deepcopy(transacts_dict_of_lists),
    ...
    for i, pred in enumerate(preds):
        ...
        for action in actions:
            ...
            if np.all(np.array(activation_list)) or single_override or double_override:
                ...
                # pdb.set_trace()                                                      # ###PAUL_del_later
                if debug_triggers:
                    triggers = {'activation_list': np.all(np.array(activation_list)),
                ...
                if (position == 'short' and action == 'long') or (position == 'long' and action == 'short'):
                    ...
                    num_transacts, position = handle_triggered_transaction(state_dict=state_dict,
                                                                           num_transacts=num_transacts,
                                                                           transacts_list=transacts_list,
                                                                           transacts=transacts,
                                                                           action=intermediate_action,
                                                                           i=i, dti=preds.index[i], price=price,
                                                                           triggers=triggers, )
...
def compute_pf_old(transacts_list):
    ...
    direction["transactions_return"] = 1 + direction["action"].shift().fillna(0) * direction[
        "price"].pct_change().fillna(0)
...
def compute_pf(transacts_list, prices_t, bps=5):
    ...
    direction = pd.DataFrame(transacts_list)[["action", "datetime", "price"]].set_index("datetime")
    direction["action"] = direction["action"].map({"short": -1, "long": 1, "exit_short": 0, "exit_long": 0})
    direction["transactions_return"] = 1 + fees * direction["action"].shift().fillna(0) * direction[
        "price"].pct_change().fillna(0)
    direction = direction.merge(prices_t, left_index=True, right_index=True, how="outer")
    direction["transactions_return"] = direction["transactions_return"].fillna(1).cumprod()
    direction["action"] = direction["action"].fillna(method="ffill").fillna(0)
...
def backtest_decision_making(price_series, signal_dict, decision_params, decision_series_name='signal', freq='min', ):
    ...
    transacts_list, transacts = run_decision_algo(preds=preds, prices=prices, requests_dict=requests_dict, )
    ...
    port_value_ts, port_return_ts, pnl = get_port_value_and_return_series(prices=prices,
                                                                          preds=preds,
                                                                          transacts_list=transacts_list,
                                                                          fee_in_percent=fee)
...
def backtest_decision_making_multiprocess(price_series, preds, freq='min', **decision_params):
    ...
    if len(transacts_list) >= max_transacts:
        ...
        port_value_ts, port_return_ts, pnl = get_port_value_and_return_series(prices=prices,
                                                                              preds=preds,
                                                                              transacts_list=transacts_list,
                                                                              fee_in_percent=decision_params['fee'])
...
def analyze_returns_at_start_and_end_of_threshold_break(signal, prices, thresh, minutes=None, hours=None, days=None):
    ...
    r__from_start_of_low = prices_matching_signal.iloc[ilocs_t + n_minutes].values\
                           / prices_matching_signal.iloc[ilocs_t].values
    ilocs_t = end_of_min_ilocs
    ilocs_t = ilocs_t[ilocs_t < signal.shape[0] - n_minutes]
    r__from_end_of_low = prices_matching_signal.iloc[ilocs_t + n_minutes].values\
                         / prices_matching_signal.iloc[ilocs_t].values

    # high threshold
    ilocs_t = start_of_max_ilocs
    ilocs_t = ilocs_t[ilocs_t < signal.shape[0] - n_minutes]
    r__from_start_of_high = prices_matching_signal.iloc[ilocs_t + n_minutes].values\
                            / prices_matching_signal.iloc[ilocs_t].values
    ilocs_t = end_of_max_ilocs
    ilocs_t = ilocs_t[ilocs_t < signal.shape[0] - n_minutes]
    r__from_end_of_high = prices_matching_signal.iloc[ilocs_t + n_minutes].values\
                          / prices_matching_signal.iloc[ilocs_t].values
    ...
    for iloc in start_of_min_ilocs[start_of_min_ilocs < signal.shape[0] - n_minutes]:
        transact_at_break_thresh_list.append({'action': 'long',
                                              'idx': iloc,
                                              'datetime': prices_matching_signal.index[iloc],
                                              'price': prices_matching_signal.iloc[iloc],
                                              'triggers': 'debug_triggers_off'})
        transact_at_break_thresh_list.append({'action': 'exit_long',
                                              'idx': iloc + n_minutes,
    ...
    for iloc in start_of_max_ilocs[start_of_max_ilocs < signal.shape[0] - n_minutes]:
        transact_at_break_thresh_list.append({'action': 'short',
                                              'idx': iloc,
                                              'datetime': prices_matching_signal.index[iloc],
                                              'price': prices_matching_signal.iloc[iloc],
                                              'triggers': 'debug_triggers_off'})
        transact_at_break_thresh_list.append({'action': 'exit_short',
                                              'idx': iloc + n_minutes,
                                              'datetime': prices_matching_signal.index[iloc + n_minutes],
                                              'price': prices_matching_signal.iloc[iloc + n_minutes],
                                              'triggers': 'debug_triggers_off'})
    ...
    for iloc in end_of_min_ilocs[end_of_min_ilocs < signal.shape[0] - n_minutes]:
        transact_at_back_under_thresh_list.append({'action': 'long',
                                                   'idx': iloc,
                                                   'datetime': prices_matching_signal.index[iloc],
                                                   'price': prices_matching_signal.iloc[iloc],
                                                   'triggers': 'debug_triggers_off'})
        transact_at_back_under_thresh_list.append({'action': 'exit_long',
                                                   'idx': iloc + n_minutes,
                                                   'datetime': prices_matching_signal.index[iloc + n_minutes],
                                                   'price': prices_matching_signal.iloc[iloc + n_minutes],
                                                   'triggers': 'debug_triggers_off'})
    ...
    for iloc in end_of_max_ilocs[end_of_max_ilocs < signal.shape[0] - n_minutes]:
        transact_at_back_under_thresh_list.append({'action': 'short',
                                                   'idx': iloc,
                                                   'datetime': prices_matching_signal.index[iloc],
                                                   'price': prices_matching_signal.iloc[iloc],
                                                   'triggers': 'debug_triggers_off'})
        transact_at_back_under_thresh_list.append({'action': 'exit_short',
                                                   'idx': iloc + n_minutes,
                                                   'datetime': prices_matching_signal.index[iloc + n_minutes],
                                                   'price': prices_matching_signal.iloc[iloc + n_minutes],
                                                   'triggers': 'debug_triggers_off'})
...
def get_feisable_transacts_list_from_time_strat(transacts_list):
    ...
    last_action = 'start'
    last_action_map = {'start': ['long', 'short'],
                       'long': ['exit_long'],
...
def make_transacts_dict_from_list(transacts_list):
    transacts = {'long': {'idxs': [], 'datetimes': [], 'prices': [], 'triggers': []},
...
def run_simple_signal_threshold_and_hold_time_based_framework(prices, signal_dict, thresh=2, days=None, hours=None,
                                                              minutes=None,
                                                              trigger_on='back_under', fee_in_percent=0):
    """
    inputs:
    ...
    transacts_list____on_break_above, \
    transacts_list____on_back_under \
        = analyze_returns_at_start_and_end_of_threshold_break(signal=signal,
    ...
    port_value_ts, port_return_ts, pnl = get_port_value_and_return_series(prices=prices_matching_signal,
                                                                          preds=signal,
                                                                          transacts_list=transacts_list,
            ...
            if np.all(np.array(activation_list)) or single_override or double_override:
                ...
                if (position == 'short' and action == 'long') or (position == 'long' and action == 'short'):
                    ...
                    num_transacts, position = handle_triggered_transaction(state_dict=state_dict,
                                                                           ...
                                                                           transacts=transacts,
                                                                           action=intermediate_action,
                                                                           i=i, dti=signal.index[i], price=price,
                                                                           triggers=triggers, )
