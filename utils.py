...
def init_ch_client(send_receive_timeout=120 * 60, max_execution_time=120 * 60):
    ch_client = CH_Client(
        host=get_secret("CH_ALGOS_DB_HOST"),
...
def wait_for_next_execution(delay_seconds, interval="1min"):
    ...
    if interval == "1min":
        ...
        next_execution_time = (
            pd.to_datetime(datetime.datetime.now()).floor("min") + timedelta(minutes=1) + timedelta(seconds=delay_seconds)
        ).to_pydatetime()
...
async def async_wait_for_next_execution(delay_seconds, interval="1min"):
    ...
    if interval == "1min":
        ...
        next_execution_time = (now + timedelta(minutes=1)).replace(second=0, microsecond=0) + timedelta(seconds=delay_seconds)
    ...
    if isinstance(date, tuple):
        ...
        if len(date) == 3:
            ...
            if output_type in [
                "pandas",
                "pd.datetime",
            ]:  # ###PAUL TODO: (datetime should imply to millisecond, which is microsecond in datetime)
        ...
        if len(date) == 6:
            ...
            if output_type in ["pandas", "pd.datetime"]:
                return pd.to_datetime(datetime.datetime(year=year, month=month, day=day, hour=hour, minute=minute, second=second))
...
def convert_symbol(symbol, in_exchange, out_exchange, params=params):
    ...
    if re.search("USD$", symbol) != None and in_exchange == "universal" and out_exchange in ["binanceus"]:
        symbol += "T"
...
def get_data_file_path(data_type, pair, date="live", port=None, signal=None, exchange=None, pair_mode="asis", params=params):
    ...
    if date in {"live"}:
        if data_type in {"trade", "trades", "price", "prices"}:
            ...
            if exchange is None:
                ...
                fp = live_data_dir + "trades_live/" + exchange + "/" + pair + "/" + exchange + "_" + pair + "_live_trades.csv"
    ...
    #     date = convert_date_format(date=date, output_type='tuple_to_day')
        ...
        if exchange is not None and port is None:
            if data_type == "price" or data_type == "prices":
                fp = live_data_dir + "price/" + exchange + "/" + pair + "/prices----" + exchange + "_" + pair + suffix
            elif data_type == "trade" or data_type == "trades":
                fp = live_data_dir + "trades_daily/" + exchange + "/" + pair + "/trades----" + exchange + "_" + pair + suffix
            elif data_type == "book":
                fp = live_data_dir + "book/" + exchange + "/" + pair + "/book----" + exchange + "_" + pair + suffix
...
def fill_trading_summary_interpolating_missing_minutes(trading_summary):
    ...
    full_index = pd.date_range(
        start=pd.Timestamp(day) + pd.Timedelta(minutes=1), end=pd.Timestamp(day) + pd.Timedelta(days=1), freq="min"
    )
...
def push_trading_summary_to_clickhouse(
    trading_summary, exchange, symbol, overwrite_mode="python_side", output="quiet", ch_client=None
):
    ...
    existing_dates = from_table_get_existing_dates_in_series(
        ch_client,
        table=table,
        exchange=exchange,
        symbol=symbol,
    ...
    if output == "verbose":
        print(
            f"there were {len(idxs_to_drop)} duplicates prevented from being entered ---- "
    ...
    num_rows_entered = ch_client.execute(
        "INSERT INTO algos_db.TradingSummary VALUES", trading_summary.reset_index().to_dict("records")
    )
...
def make_day_of_trading_summary_push_to_db(exchange, symbol, date, overwrite_mode="python_side", ch_client=None):
    ...
    push_trading_summary_to_clickhouse(
        trading_summary=trading_summary, exchange=exchange, symbol=symbol, overwrite_mode=overwrite_mode, ch_client=ch_client
    )

    return trading_summary


def make_trading_summary_from_trades_in_batches(
    exchange, symbol, date=None, start_date=None, end_date=None, freq="W", overwrite_mode="python_side", ch_client=None
):
    ...
    if len(df_list) != 0:
        ...
        print(
            f" - final data clean ---- pre_cleaned_nans: {pre_cleaned_nans} "
    ...
    push_trading_summary_to_clickhouse(
        trading_summary=all_trading_summary, exchange=exchange, symbol=symbol, overwrite_mode=overwrite_mode, ch_client=ch_client
    )
...
def update_trading_summary_table(
    exchange="binance", symbol="BTC-USDT", output="quiet", overwrite_mode="python_side", ch_client=None
):
    ...
    last_trading_summary_datetime = ch_client.execute(query)[0][0] - timedelta(
        minutes=60
    )  # ###PAUL_del_later ---- the time delta is not needed but we use it to ensure overwrite in push is working correctly
    ...
    if new_trading_summary_info.shape[0] != 0:
        push_trading_summary_to_clickhouse(
            trading_summary=new_trading_summary_info,
            exchange=exchange,
            symbol=symbol,
...
def add_signal_name_to_foreign_key_table(signal_name):
    ...
    if signal_id is not False:
        ...
        ch_client.execute(
            f"""INSERT INTO algos_db.AlgosSignalNames (signal_id, signal_name) VALUES""", [(signal_id, signal_name)]
        )
...
def bulk_insert_signal_observations(signal_name, signal):
    ...
    print(
        f"dont want this to work without adding the logic to add trading summary which checks "
    ...
    if signal_id:  # the above query returns False if there is no signal name matching
        ...
        num_rows_entered = ch_client.execute("INSERT INTO algos_db.AlgosSignals VALUES", signal.reset_index().to_dict("records"))
    else:
...
def update_signals_table(signal, signal_name=None, signal_id=None, overwrite=False, ch_client=None):
    ...
    num_rows_entered = ch_client.execute(
        "INSERT INTO algos_db.AlgosSignals VALUES", signal_to_push.reset_index().to_dict("records")
    )
...
def insert_trades(ccxt_trades):
    ...
    for trade in ccxt_trades:
        ...
        trade = {
            "timestamp": trade["datetime"],  # ###PAUL TODO: gon need to verify this
            "id": trade["id"],
            "price": trade["price"],
            "amount": trade["amount"],
