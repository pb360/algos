...
def directory_check_for_portfolio_data(params):
    ...
    fps_needed.extend([port_path + 'last_check.txt', port_path + 'open_orders.csv', ])

    for exchange in params['port']['exchanges']:
        exchange_open_order_fp = get_data_file_path(data_type='open_orders',
                                                    pair=None,
                                                    date='live',
                                                    port=params['port']['port_name'],
                                                    exchange=exchange, )
        exchange_closed_order_dir = get_data_file_path(data_type='closed_orders',
                                                       pair=None,
                                                       date='path',
                                                       port=params['port']['port_name'],
                                                       exchange=exchange, )
...
def cache_state_dict(state_dict, params):
    ...
    if 'port' in keys:
        state_dict_fp = get_data_file_path(data_type='state_dict',
                                           pair='None',
                                           date='live',
                                           port=params['port']['port_name'],
...
def update_trading_summary(pair, trading_summaries, state_dict, params, ch_client=None):
    ...
    trading_summary = query_trading_summary(exchange=params['port']['exchange'],
                                            symbol=pair,
...
def get_usd_value_of_port_holdings_and_positions(trading_summaries, ch_client, state_dict, params):
    ...
    for symbol in params['port']['set_of_assets_in_port']:
        state_dict['port_holdings_dict'][symbol] = state_dict['account_balances'][
            symbol]  # gets holdings for that base asset / ticker
        ...
        position.update({'strategy': 'peak_bottom____spot',
                         'timestamp': ts,
    ...
    for pair in params['port']['pairs_traded']:
        ...
        if portfolio_method == 'LS':
            ...
            if pair not in state_dict['LS']:
                ...
                initial_TUSD = 50
                initial_BTC = max_base_qty_for_pair - initial_TUSD / base_priced_in_quote  # should be `base_priced_in_usd` but cant do till have TUSD price correct
                initial_BTC_in_TUSD = initial_BTC * base_priced_in_quote  # should be `base_priced_in_usd` but cant do till have TUSD price correct
            ...
            # not the first run for the pair
                ...
                state_dict['LS'][pair]['BTC_before_trade'] = \
                    state_dict['LS'][pair]['initial_quote'] \
                    + state_dict['LS'][pair]['mocked_port_BTC_target_current']  # reset, so value from prior iter
                ...
                state_dict['LS'][pair]['TUSD_before_trade'] = \
                    state_dict['LS'][pair]['TUSD_before_trade_prior_iter'] \
                    + base_priced_in_quote * (state_dict['LS'][pair]['BTC_before_trade_prior_iter']
                                              - state_dict['LS'][pair]['BTC_before_trade'])
            ...
            if state_dict['desired_position'][pair] == 'short':
                # TODO: (BELOW) should be `base_priced_in_usd` but cant do till have TUSD price correct
                mocked_port_BTC_target_current = \
                    -state_dict['LS'][pair]['mocked_port_value_before_trade'] / base_priced_in_quote
            ...
            state_dict['bag_desired'][pair]['base'] = \
                mocked_port_BTC_target_current + state_dict['LS'][pair]['initial_quote']
            state_dict['bag_desired'][pair]['base_in_quote'] = \
                base_priced_in_quote * state_dict['bag_desired'][pair]['base']
            state_dict['bag_desired'][pair]['base_in_usd'] = \
                base_priced_in_usd * state_dict['bag_desired'][pair]['base']

            state_dict['bag_desired'][pair]['quote'] = \
                state_dict['LS'][pair]['TUSD_before_trade'] \
                + (state_dict['LS'][pair]['BTC_before_trade'] - state_dict['bag_desired'][pair]['base']) \
                / base_priced_in_usd
            ...
            # TODO: ^^^^ consider multiple pairs sharing the same quote (will make multiple position rows in the df
                ...
                state_dict['LS'][pair]['mocked_port_TUSD_target_current'] = \
                    state_dict['LS'][pair]['mocked_port_value_before_trade']
                base_qty = 0
                quote_qty = state_dict['LS'][pair]['mocked_port_TUSD_target_current']
            else:
            ...
            pair_position = {'base': {'size': base_qty,
                                      'price': base_priced_in_quote,  # todo... should be done in USD but dont have yet
                                      'symbol': 'BTC', },  # TODO parameterize
                             'quote': {'size': quote_qty,
                                       'price': quote_priced_in_usd,
                                       'symbol': 'TUSD', },  # TODO parameterize
                             }
            ...
            for side in ['base', 'quote']:
                position = deepcopy(params['port']['positions_table_info'])
                position.update({'strategy': 'peak_bottom____long_short',
                                 'timestamp': ts,
                                 'leg_group_id': int(ts.timestamp() * 1000),
...
def update_account_balances(ccxt_client, state_dict):
    ...
    account_balances_t = ccxt_client.fetch_balance()  # ccxt returns dict of dicts:  'XRP': {'free': 0.0, 'used': 0.0, 'total': 0.0}
...
def update_desired_positions_dict(trading_summaries, signal_dfs_dict, state_dict, params):
    ...
    for pair in params['port']['pairs_traded']:
        triggered_actions = decide_live(state_dict=state_dict,
...
def process_placed_order(placed_order_res, state_dict, params):
    ...
    #
    open_order_fp = get_data_file_path(data_type='open_orders',
                                       pair='None',
                                       date='live',
                                       port=params['port']['port_name'],
                                       exchange=params['port']['exchange'],
...
def place_order(ccxt_client, B_or_S, pair, o_type, base_qty, price, state_dict, params):
    ...
    if o_type.lower() == 'limit':
        ...
        order_res = ccxt_client.create_limit_order(symbol=symbol,
                                                   side=B_or_S,
                                                   amount=base_qty,
                                                   price=price)
...
def write_closed_order(pair, response, params):
    ...
    daily_trade_fp = get_data_file_path('closed_order', pair, date='live', port=params['port']['port_name'],
                                        exchange=params['port']['exchange'])
...
def remove_order_from_open_tracking(tuple_key, response, state_dict, params):
    ...
    #
    open_order_fp = get_data_file_path(data_type='open_orders', pair=None, date='live',
                                       port=params['port']['port_name'],
                                       exchange=params['port']['exchange'])
    ...
    for open_order in all_opens:
        ...
        if base_asset in params['port']['assets_in_port'] or quote_asset in params['port']['assets_in_port']:
            ...
            if order_key in state_dict['order_open_dict']:
                ...
                print(f"closing order on bot start for "
...
def check_for_orders_and_close_unrelated(ccxt_client, state_dict, params):
    ...
    for res in open_orders_res_list:
        ...
        universal_symbol = convert_symbol(exchange_symbol, in_exchange=params['port']['exchange'],
                                          out_exchange='universal')
...
def check_for_closed_orders(ccxt_client, state_dict, params):
    ...
    for key, value in temp_open_order_dict.items():
        ...
        exchange_symbol = convert_symbol(universal_symbol, in_exchange='universal',
                                         out_exchange=params['port']['exchange'])
...
def print_update_on_bot(state_dict, signal_dfs_dict, params):  # utility that only uses state dict so it goes here...
    ...
    print(f" - ALGOS - LIVE BOT - portfolio iter: {state_dict['iter_count']}"
    ...
    print(f"        - BTC: {state_dict['LS']['BTC-TUSD']['mocked_port_BTC_target_current']} -- "
...
def trading_summary_collection_check(trading_summaries, state_dict, params):
    ...
    if all_pairs_empty:
        ...
        print(f"    - `all_pairs_empty` ---- leaving to see if it happens # ###PAUL_del_later \n"
...
def handle_ordering_after_state_update(ccxt_client, trading_summaries, state_dict, params):
    ...
    for pair in params['port']['pairs_traded']:
        ...
        if order_price is not None:  # we are ordering
            ...
            if B_or_S == 'buy':
                max_orderable_qty = \
                    state_dict['bag_actual'][pair]['quote'] / state_dict['last_prices_by_pair_in_quote'][pair]
            if B_or_S == 'sell':
            ...
            print(f"order_stage_3 ---- PLACING LIMIT ORDER ---- {B_or_S}  -  {qty}  -  {pair}  ${order_price} "
                  f"---- in port_name: {params['port']['port_name']}")

            try:
                order_res = place_order(ccxt_client=ccxt_client,
                                        B_or_S=B_or_S,
                                        pair=pair,
                                        o_type='limit',
                                        base_qty=qty,
...
def print_signal_update(state_dict, params):
    print(f"for signal update iter #: {state_dict['num_total_signal_updates']} ---- and "
...
def reassess_bags_and_orders(ccxt_client, ch_client, trading_summaries, signal_dfs_dict, state_dict, params):
    ...
    handle_ordering_after_state_update(ccxt_client=ccxt_client,
